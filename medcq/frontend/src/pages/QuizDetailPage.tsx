...
const QuizDetailPage: React.FC = () => {
  ...
  const fetchQuiz = useCallback(async () => {
    ...
    try {
      ...
      setError(getErrorMessage(err, 'Unable to load quiz details. Please try again later.'));
    } finally {
      setIsLoading(false);
    }
  }, [quizId]);
...
useEffect(() => {
  ...
  const trackPageView = () => {
    console.log('Analytics: Quiz Detail View', {
      quizId,
  ...
  // Track start quiz events
  const handleStartQuiz = () => {
    // This would be replaced with your actual analytics implementation
    console.log('Analytics: Start Quiz Click', {
      quizId,
      timestamp: new Date().toISOString(),
    });
    ...
    <Layout title={quiz ? `Quiz: ${quiz.title}` : 'Quiz Details'}>
      ...
      {!isLoading && !error && quiz && (
        <>
          <PageHeader
            title={quiz.title}
            subtitle={`${typeof quiz.subject === 'string' ? quiz.subject : quiz.subject.name}${
              quiz.yearLevel ? ` • Year ${quiz.yearLevel}` : ''
            }`}
            actions={
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-12">
              ...
              <div className="lg:col-span-2">
                <Section title="About this Quiz">
                  ...
                  {/* Question preview */}
                  <div className="mt-8" role="region" aria-labelledby="sample-questions-heading">
                    <h3
